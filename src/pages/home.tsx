  ...
  return (
    <section className="w-full pt-[118px]">
      <motion.div initial={{ opacity: 0, scale: 1.02 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, ease: "easeOut" }} className="w-full relative">
        <img src={heroImagePath} alt="Annadata Agri & Seeds — Keshav Meena" className="w-full h-auto block" />
...
function OnlineDhanBookingSection() {
  ...
  return (
    <section className="py-14 md:py-20 bg-foreground relative overflow-hidden">
      <div className="absolute inset-0 opacity-[0.04] pointer-events-none"
        style={{ backgroundImage: "radial-gradient(circle at 1px 1px, #F9A825 1px, transparent 0)", backgroundSize: "30px 30px" }} />
      ...
      <div className="container mx-auto px-4 md:px-6 max-w-3xl relative z-10">
        <div className="text-center mb-8">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
            className="inline-flex items-center gap-2 bg-green-500/25 text-green-300 font-bold px-4 py-1.5 rounded-full mb-3 text-xs font-hindi border border-green-500/30">
            🌱 खरीफ सीजन 2026 — धान + सोयाबीन बुवाई शुरू! अभी Booking करें
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            className="inline-flex items-center gap-2 bg-secondary/20 text-secondary font-bold px-4 py-2 rounded-full mb-3 text-sm font-hindi ml-2">
            🏠 Online Booking
          </motion.div>
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}
            className="text-3xl md:text-4xl font-hindi font-black text-white leading-snug">
            धान • सोयाबीन • दवाई<br />
            <span className="text-secondary">घर तक मँगाएं!</span>
          </motion.h2>
          <p className="text-white/60 font-hindi mt-2 max-w-xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <div className="bg-white/5 border border-green-400/25 rounded-2xl p-4 text-center">
          ...
          <div className="bg-white/5 border border-yellow-400/25 rounded-2xl p-4 text-center">
            <div className="text-3xl mb-1">🌿</div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 mb-8">
          {features.map((f, i) => (
            <motion.div key={i} initial={{ opacity: 0, scale: 0.85 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
              ...
              className="flex flex-col items-center gap-1.5 bg-white/5 border border-secondary/20 rounded-2xl py-3 px-2 text-center">
              <span className="text-2xl">{f.emoji}</span>
              <span className="text-white/80 font-hindi text-xs font-semibold leading-tight">{f.text}</span>
            </motion.div>
          ))}
        </div>

        <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          className="bg-white/5 rounded-3xl p-6 md:p-8 border-2 border-secondary/30 shadow-2xl space-y-4"
